...
<?php $tg_postbooking = (isset($_REQUEST['action']) && $_REQUEST['action'] == 'tg_postbooking') ? 'tg_postbooking' : ''; ?>
...
<html>
	...
	<body class="rezgo-booking-payment-body">
		...
		<script>
			jQuery(document).ready(function($){
				...
				<?php if (isset($_REQUEST['rezgoAction']) && $_REQUEST['rezgoAction'] == 'return') { ?>
					creditConfirm("<?php echo esc_js($_REQUEST['token']); ?>");
				<?php } ?>
			});
		</script>
	
		<form method="post" id="payment" action="https://process.rezgo.com/form" autocomplete="off">
			<input type="hidden" name="return" value="<?php echo home_url(); ?>?rezgo=1&mode=booking_payment&action=<?php echo $tg_postbooking ?? ''; ?>&rezgoAction=return&" />

		<div class="cc-form-container">
			<div class="cc-field-container form-group">
				<input id="name" placeholder="Name" class="form-control" name="name" maxlength="20" type="text" value="<?php echo esc_attr($site->requestStr('name')); ?>" placeholder="" autocomplete="off" required>
			</div>
			
			<div class="cc-field-container form-group">
				<input id="pan" placeholder="Card Number" class="form-control" name="pan" type="text" value="<?php echo esc_attr($site->requestStr('pan')); ?>" pattern="[0-9]*" inputmode="numeric" placeholder="" autocomplete="off" required>
			...
			<style>
				<?php if($site->getCVV() || $tg_postbooking){ ?>
					input#exp{
						width: 100%;	
					}
				<?php } else { ?> 
			...
			<div class="cc-field-container form-group">
				<input id="exp" placeholder="MM/YY" class="form-control required" name="exp" type="text" pattern="[0-9]*" inputmode="numeric" placeholder="" autocomplete="off" required>
			...
			<?php if($site->getCVV() || $tg_postbooking) { ?>
				<div class="cc-field-container form-group">
					<input id="rezgo-cvv" placeholder="CVV" class="form-control required" name ="cvv" type="text" pattern="[0-9]*" inputmode="numeric" placeholder="" autocomplete="off" required>
					...
					<div class="popover__wrapper">
						...
						<div class="popover__content">
							<img class="cvv-img" alt="what is cvv?" src="<?php echo esc_html($site->path); ?>/img/cvv_cards.png">
						</div>
					</div>
				</div>
			<?php } ?>
	...
	window.onload = function () {
		...
		// CC ICONS
		let card = '<i class="fa fa-credit-card" style="transform:scale(1.1, 0.95)translateX(2px); transform-origin:center;"></i>';
		let amex = '<i class="fa fa-cc-amex"></i>';
		let visa = '<i class="fa fa-cc-visa"></i>';
		let discover = '<i class="fa fa-cc-discover"></i>';
		let mastercard = '<i class="fa fa-cc-mastercard"></i>';
